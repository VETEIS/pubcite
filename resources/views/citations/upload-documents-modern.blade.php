...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-all duration-200 group cursor-pointer"
            ...
            <div class="text-center">
                ...
                <p class="text-sm text-gray-600 mb-4">Copy of the citing article (PDF)</p>
                <div class="text-sm text-maroon-600 font-medium mb-2 truncate" x-text="displayName || 'Click to upload'"></div>
                <p class="text-xs text-gray-500">Max 20MB • PDF only</p>
            ...
            <input type="file" name="citing_article" accept=".pdf" class="hidden" x-ref="citingArticle" required
                   @change="handleFileUpload($event, 'citing_article', $data)">
        </div>
        ...
        <div class="bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-all duration-200 group cursor-pointer"
            ...
            <div class="text-center">
                ...
                <p class="text-sm text-gray-600 mb-4">Copy of the cited article (PDF)</p>
                <div class="text-sm text-maroon-600 font-medium mb-2 truncate" x-text="displayName || 'Click to upload'"></div>
                <p class="text-xs text-gray-500">Max 20MB • PDF only</p>
            ...
            <input type="file" name="cited_article" accept=".pdf" class="hidden" x-ref="citedArticle" required
                   @change="handleFileUpload($event, 'cited_article', $data)">
        </div>
    </div>
    ...
    <div class="bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200 p-6 shadow-sm" x-data="uploadProgress()">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600">Citing Article</span>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded-full" :class="uploadStatus.citing_article ? 'bg-green-500' : 'bg-gray-300'"></div>
                    <span class="text-xs" :class="uploadStatus.citing_article ? 'text-green-600' : 'text-gray-500'" 
                          x-text="uploadStatus.citing_article ? 'Uploaded' : 'Not uploaded'"></span>
                </div>
            </div>
            <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600">Cited Article</span>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded-full" :class="uploadStatus.cited_article ? 'bg-green-500' : 'bg-gray-300'"></div>
                    <span class="text-xs" :class="uploadStatus.cited_article ? 'text-green-600' : 'text-gray-500'" 
                          x-text="uploadStatus.cited_article ? 'Uploaded' : 'Not uploaded'"></span>
                </div>
            </div>
        </div>
    </div>
...
<script>
// Global upload status tracking for citations
window.uploadStatus = {
    recommendation_letter: false,
    citing_article: false,
    cited_article: false
};
...
function uploadFile(file, fieldName, componentData) {
    ...
    // Start upload
    xhr.open('POST', '/citations/upload-temp', true);
    xhr.send(formData);
}
