                        ...
                        >
                            ...
                            <div class="flex border-b border-gray-200 mb-6">
                                ...
                                <button type="button" 
                                    ...
                                    :class="[
                                        ...
                                        !(tabCompletion.incentive && tabCompletion.recommendation && tabCompletion.upload) ? 'opacity-50 cursor-not-allowed' : ''
                                    ]"
                                    :disabled="!(tabCompletion.incentive && tabCompletion.recommendation && tabCompletion.upload)"
                            ...
                            <div class="min-h-[500px]">
                                ...
                                <div x-show="activeTab === 'upload'" class="space-y-6">
                                    @include('citations.upload-documents-modern')
                                </div>
                            ...
                            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                                <button type="button" 
                                    ...
                                    @click="switchTab(activeTab === 'incentive' ? 'incentive' : (activeTab === 'recommendation' ? 'incentive' : (activeTab === 'upload' ? 'recommendation' : 'upload')))">
                                    Previous
                                </button>
                                ...
                                <div class="flex items-center gap-3">
                                    <button type="button" 
                                        ...
                                        @click="switchTab(activeTab === 'incentive' ? 'recommendation' : (activeTab === 'recommendation' ? 'upload' : 'review'))"
            ...
            return {
                ...
                showError(message) {
                 ...
                 async saveDraft() {
                     ...
                     try {
                         ...
                         console.log('Saving citation draft - collecting form data for current tab:', this.activeTab);
                         
                         // Only collect fields from the current tab
                         let currentTabFields = [];
                         if (this.activeTab === 'incentive') {
                             currentTabFields = ['name', 'rank', 'college', 'bibentry', 'issn', 'doi', 'scopus', 'wos', 'aci', 'faculty_name', 'center_manager', 'dean_name', 'date'];
                         } else if (this.activeTab === 'recommendation') {
                             currentTabFields = ['rec_collegeheader', 'date', 'rec_faculty_name', 'rec_citing_details', 'rec_indexing_details', 'rec_dean_name'];
                         } else if (this.activeTab === 'upload') {
                             currentTabFields = ['recommendation_letter', 'citing_article', 'cited_article'];
                         }
                         
                         console.log('Current citation tab fields to collect:', currentTabFields);
                        ...
                        console.log('Sending citation draft save request...');
                        ...
                        console.error('Citation draft save error:', error);
                ...
                validateCurrentTab() {
                    ...
                    if (currentTab === 'incentive') {
                        fields = ['name', 'rank', 'college', 'bibentry', 'issn', 'doi', 'scopus', 'wos', 'aci', 'faculty_name', 'center_manager', 'dean_name'];
                    } else if (currentTab === 'recommendation') {
                        fields = ['rec_collegeheader', 'date', 'rec_faculty_name', 'rec_citing_details', 'rec_indexing_details', 'rec_dean_name'];
                    } else if (currentTab === 'upload') {
                        fields = ['recommendation_letter', 'citing_article', 'cited_article'];
                    }
                    
                    console.log(`Validating ${currentTab} tab with fields:`, fields);
                    ...
                    fields.forEach(field => {
                        ...
                        if (!element) {
                            console.log(`Field not found for validation: ${field}`);
                            return;
                        }
                        
                        let fieldValid = false;
                        if (element.type === 'radio') {
                            fieldValid = document.querySelector(`[name="${field}"]:checked`) !== null;
                        } else if (element.type === 'checkbox') {
                            const checkboxes = document.querySelectorAll(`[name="${field}"]`);
                            fieldValid = Array.from(checkboxes).some(cb => cb.checked);
                        } else if (element.type === 'file') {
                            fieldValid = element.files && element.files.length > 0;
                        } else {
                            fieldValid = element.value.trim() !== '';
                        }
                        
                        console.log(`Validating ${currentTab} field ${field}: ${element.value} (valid: ${fieldValid})`);
                        
                        if (!fieldValid) {
                            isValid = false;
                            if (!firstIncompleteField) firstIncompleteField = element;
                            element.classList.add('ring-2', 'ring-red-500', 'ring-offset-2');
                            setTimeout(() => element.classList.remove('ring-2', 'ring-red-500', 'ring-offset-2'), 3000);
                        }
                    });
                    
                    console.log(`${currentTab} tab validation result: ${isValid}`);
                    
                    if (!isValid && firstIncompleteField) {
                ...
                canProceedToTab(targetTab) {
                    const tabOrder = ['incentive', 'recommendation', 'upload', 'review'];
                    const currentIndex = tabOrder.indexOf(this.activeTab);
                    const targetIndex = tabOrder.indexOf(targetTab);
                    
                    console.log(`Can proceed to ${targetTab}? Current: ${this.activeTab}`);
                    
                    if (targetIndex <= currentIndex) return true;
                    
                    // Only check tabs up to the target tab, not all tabs
                    for (let i = currentIndex; i < targetIndex; i++) {
                        const tabName = tabOrder[i];
                        console.log(`Checking if tab ${tabName} is complete...`);
                        
                        // Check only the specific tab that needs to be complete
                        const isComplete = this.checkSpecificTabCompletion(tabName);
                        console.log(`Tab ${tabName} completion: ${isComplete}`);
                        
                        if (!isComplete) {
                            console.log(`Cannot proceed - tab ${tabName} is not complete`);
                            return false;
                        }
                    }
                    console.log('All required tabs are complete, can proceed');
                    return true;
                },
                switchTab(targetTab) {
                    if (this.canProceedToTab(targetTab)) {
                        this.activeTab = targetTab;
                        // Update all tab indicators when switching tabs
                        this.updateAllTabCompletion();
                    } else {
                        this.showError('Please complete the current tab before proceeding.');
                    }
                },
                checkTabCompletion() {
                ...
                checkCurrentTabCompletion() {
                    const currentTab = this.activeTab;
                    console.log(`Checking current citation tab completion for: ${currentTab}`);
                    
                    if (currentTab === 'incentive') {
                        const incentiveFields = ['name', 'rank', 'college', 'bibentry', 'issn', 'doi', 'scopus', 'wos', 'aci', 'faculty_name', 'center_manager', 'dean_name'];
                        ...
                        const recommendationFields = ['rec_collegeheader', 'date', 'rec_faculty_name', 'rec_citing_details', 'rec_indexing_details', 'rec_dean_name'];
                        ...
                        const uploadFields = ['recommendation_letter', 'citing_article', 'cited_article'];
                    ...
                    console.log(`Current citation tab (${currentTab}) completion:`, this.tabCompletion[currentTab]);
                },
                updateAllTabCompletion() {
                    console.log('=== UPDATING ALL TAB COMPLETION (CITATIONS) ===');
                    
                    // Check incentive tab
                    const incentiveFields = ['name', 'rank', 'college', 'bibentry', 'issn', 'doi', 'scopus', 'wos', 'aci', 'faculty_name', 'center_manager', 'dean_name'];
                    console.log('Checking citation incentive fields:', incentiveFields);
                    
                    this.tabCompletion.incentive = incentiveFields.every(field => {
                        return this.isFieldValid(field);
                    });
                    
                    console.log(`Citation incentive tab completion: ${this.tabCompletion.incentive}`);
